<div class="container-fluid">
	...
	<div class="table-responsive">
		...
		<td>
			<form method="post" name="cari" action="dasar.php?page=cari_cara&data=<?php echo $data1;?>">
				<b>Cari Data :</b></span><input type="" class="form-control" name="data" placeholder="Masukkan ID atau Cara Resep"><br>
			</form>
		</td>
		</tr>
		</table>
	...
	<!------------------------- TABS MACAM DATA ---------------------------->
		<ul class="nav nav-tabs nav-justified">
			...
			<li class="active"><a href="#">CARA RESEP</a></li>
		...
		<table border="0" align="center" class="table table-hover">
			...
			<?php
				...
				$query="select * from cara_resep WHERE id_resep LIKE '%$data%' OR cara_resep LIKE '%$data%' limit $posisi,$batas ";
			...
			<tbody>
				<tr>
					...
					<td align='justify'><?php echo $r['cara_resep']; ?></td>
					<!--kelola-->
					<td align='center'>
						<a onclick="return confirm('Apakah Anda yakin akan menghapus data ini?')" href="dasar.php?page=hapus_cara&id=<?php echo $r['id_resep']; ?>"><span class="glyphicon glyphicon-trash"></span></a>
					</td>
		...
		<div class='page'>
			<ul class="pagination">
				...
				<?php 	
					...
					$sql_paging = mysql_query("select * from cara_resep WHERE id_resep LIKE '%$data%' OR cara_resep LIKE '%$data%'");
					$jmldata = mysql_num_rows($sql_paging) or die (mysql_error());
					$jumlah_halaman = ceil($jmldata / $batas);
					for($i = 1; $i <= $jumlah_halaman; $i++)
					{
						echo "<font style='padding:10px;'><a href=dasar.php?page=cari_cara&halaman=$i'> $i </a></font>";
					}
					$i=$i-1;
				?>
				</li>
