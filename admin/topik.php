	...
	<!------------------------- TABS MACAM DATA ---------------------------->		
		...
		<table border="0" align="center" class="table table-hover">
			...
			<?php
				...
				$query="select * from topik limit $posisi,$batas ";
				$sql_tampil_data=mysql_query($query) or die(mysql_error());
				while($r=mysql_fetch_array($sql_tampil_data))
				{ // Start looping table row
				$id = $r['id_topik'];
				$commentquery = mysql_query("SELECT * FROM komentar_topik WHERE id_topik='$id'") or die(mysql_error());
		...
		<div class='page'>
			<ul class="pagination">
				...
				<?php 	
					...
					{
						echo "<font style='padding:10px;'><a href=dasar.php?page=topik&halaman=$i&$no> $i </a></font>";
					}
					$i=$i-1;
				?>
				</li>
