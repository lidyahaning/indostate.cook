<div class="container-fluid">
	...
	<div class="table-responsive">
		...
		<td>
			<form method="post" name="cari" action="dasar.php?page=cari_komentar_resep">
	...
	<!------------------------- TABS MACAM DATA ---------------------------->
		<ul class="nav nav-tabs nav-justified">
			...
			<li class="active"><a href="#">KOMENTAR RESEP</a></li>
		...
		<table border="0" align="center" class="table table-hover">
			...
			<?php
				...
				$query="select * from komentar_resep WHERE id_resep LIKE '%$data%' OR id_anggota LIKE '%$data%' OR isi_komentar LIKE '%$data%' limit $posisi,$batas ";
			...
			<tbody>
				<tr>
					...
					<!--kelola-->
					<td align='center'>
						<a onclick='return confirm('Apakah Anda yakin akan menghapus data ini?')" href="dasar.php?page=hapus_komentar_resep&id=<?php echo $r['id_anggota']; ?>"><span class="glyphicon glyphicon-trash"></span></a>
					</td>
		...
		<div class='page'>
			<ul class="pagination">
				...
				<?php 	
					...
					$sql_paging = mysql_query("select * from komentar_resep WHERE id_resep LIKE '%$data%' OR id_anggota LIKE '%$data%' OR isi_komentar LIKE '%$data%'");
					$jmldata = mysql_num_rows($sql_paging) or die (mysql_error());
					$jumlah_halaman = ceil($jmldata / $batas);
					for($i = 1; $i <= $jumlah_halaman; $i++)
					{
						echo "<font style='padding:10px;'><a href=dasar.php?page=komentar_resep&halaman=$i&$no> $i </a></font>";
					}
					$i=$i-1;
				?>
				</li>
