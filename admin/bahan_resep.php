	...
	<div class="table-responsive">
		...
		<td>
			<form method="post" name="cari" action="dasar.php?page=cari_bahan">
				<b>Cari Data :</b></span><input type="" class="form-control" name="data" placeholder="Masukkan ID atau Bahan Resep"><br>
			</form>
		</td>
		</tr>
		</table>
	...
	<!------------------------- TABS MACAM DATA ---------------------------->
		...
		<table border="0" align="center" class="table table-hover">
			...
			<?php
				...
				$query="select * from bahan_resep limit $posisi,$batas ";
			...
			<tbody>
				<tr>
					...
					<!--kelola-->
					<td align='center'>
						<a onclick="return confirm('Apakah Anda yakin akan menghapus data ini?')" href="dasar.php?page=hapus_bahan&id=<?php echo $r['id_resep']; ?>&bahan=<?php echo $r['bahan_resep']; ?>"><span class="glyphicon glyphicon-trash"></span></a>
					</td>
		...
		<div class='page'>
			<ul class="pagination">
				...
				<?php 	
					...
					$sql_paging = mysql_query("select bahan_resep from bahan_resep");
					$jmldata = mysql_num_rows($sql_paging) or die (mysql_error());
					$jumlah_halaman = ceil($jmldata / $batas);
					for($i = 1; $i <= $jumlah_halaman; $i++)
					{
						echo "<font style='padding:10px;'><a href=dasar.php?page=bahan_resep&halaman=$i&$no> $i </a></font>";
					}
					$i=$i-1;
				?>
				</li>
